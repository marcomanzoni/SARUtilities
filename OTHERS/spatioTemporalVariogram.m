function [spatioTemporalVariogram,spatialAxis, temporalAxis] = spatioTemporalVariogram(dataCube,dates, cellSize, maxSpatialOffset)
...
for ii = 0:totalImages-1
    ...
    temporalShiftedData           = circshift(newDataCube, ii, 3);
    
    % Delete the portion that circularly shift at the top of the stack
    temporalShiftedData(:,:,1:ii) = NaN;
    ...
    for jj = 0:maxSpatialOffset
        ...
        for tt = 1:size(shiftMatrix, 1)
            ...
            if shiftDim == 1
                shiftVector = zeros(shiftAmount*2+1,1);
            else
                shiftVector = zeros(1,shiftAmount*2+1);
            end
            ...
            shiftVector = shiftVector > 0;
            

            %shiftedData     = circshift(temporalShiftedData, shiftAmount, shiftDim);
            shiftedData     = convn(temporalShiftedData, shiftVector, 'same');
            ...
            difference = cat(3, difference, (newDataCube - shiftedData).^2);
        end
